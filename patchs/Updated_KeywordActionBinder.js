...
function KeywordActionBinder() {
  ...
  // Consolidating loading state logic for `resetTimer` and other states
  useEffect(() => {
    ...
    const savedResetTimer = localStorage.getItem('resetTimer');

    // Properly loading resetTimer from localStorage
    if (savedResetTimer !== null) {
      setResetTimer(JSON.parse(savedResetTimer));
    }
  ...
  // Saving resetTimer to localStorage whenever it changes
  useEffect(() => {
    localStorage.setItem('resetTimer', JSON.stringify(resetTimer)); // Saving resetTimer state to localStorage
  }, [resetTimer]);

  // Saving other states to localStorage
  useEffect(() => {
  ...
  // Export bindings to a JSON file
  ...
  // Send chat message with keyword actions
  ...
  const addOrUpdateAction = () => {
    ...
    // Update resetTimer when adding or updating action
    localStorage.setItem('resetTimer', JSON.stringify(resetTimer));
    ...
    newActions.forEach(newAction => {
      if (resetTimer && activeTimers[newAction.keyword]) {
        ...
        clearTimeout(activeTimers[newAction.keyword]); // Reset timer if resetTimer is enabled
      } else if (!resetTimer) {
        // If resetTimer is false, create a new timer for the keyword
