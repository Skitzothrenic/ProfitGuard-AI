...
ipcMain.on('chat-message', (event, data) => {
  ...
  for (const action of triggered) {
    switch (action.type) {
      case 'Play Sound':
        if (action.file && typeof action.file === 'string' && action.file.startsWith('data:audio')) {
          console.log('[Main] Playing base64-encoded sound');
          mainWindow.webContents.send('play-sound', action.file);
  ...
  mainWindow.webContents.send('start-timer', {
    ...
    reset: action.reset !== false  // default true if missing
  });
  break;
